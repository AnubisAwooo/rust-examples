...
impl fmt::Debug for MiniString {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        println!("debug: {} {:?}", self.len, self.data);
...
impl MyString {
    fn push_str(&mut self, string: &str) {
        match self {
            Self::Standard(s) => s.push_str(string),
            Self::Inline(s) => {
                let e = s.len();
                let e2 = string.len();
                if e + e2 > MINI_STRING_MAX_LEN {
                    let mut s2 = String::from(&s[..]);
                    s2.push_str(string);
                    *self = MyString::Standard(s2);
                } else {
                    (&mut s.data[e..e + e2]).copy_from_slice(string.as_bytes());
                    s.len += e2 as u8;
                }
            }
        }
